...
function WCIncrease(){
        ...
        if(wordCount  < 6){
            clearInterval(pattern);
            ph = 0;
            let patId = "pat" + String(Math.floor(Math.random()*3)+1)+"()";//change later once more pats
            console.log(patId);
...
function pat2(){//center squeeze, it isnt efficientyl written but i dont care
    switch(ph){
        case 0:
            for(let i  = 0; i < 4; i++){
...
document.getElementById("container1").addEventListener('mouseleave', () => {
    if(!gameWon && !gameLost){
        document.getElementById("infoText").innerHTML= "Atgriežiet kursoru uz klavietūras!"
        blankWord();
        clearInterval(pattern);
        ph = 0;
    }
