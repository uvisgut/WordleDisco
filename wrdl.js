...
function press (x){
    if (letterCount <5){
        line[letterCount] = x;//letterCount is the right position
        let lid =  "l"+String(wordCount)+String(letterCount);
        //console.log(lid);
        document.getElementById(`${lid}`).innerHTML = `${x.toUpperCase()}`;
        letterCount = Math.min(letterCount + 1, 5);
        //console.log(letterCount);
    }
}
let letterToNumberDict = {
    'q': "00",
    'w': "01",
    'e': "02",
    'r': "03",
...
function determineID(letter){
    return(letterToNumberDict[letter]);
}
let dict = {
    "00": 'white',
    "01": 'white',
...
function restoreLetterButtonColor(){
    for (let key in dict) {
          let value = dict[key];
...
function enterWord(){//this entire function is inefficient but I do not care
    let inputWord = line.join("")
    if(wordsArray.includes(inputWord)){
        for(let i = 0; i <5; i++){
            //console.log(lid);

            letter = line[i];
            let lid =  "l"+String(wordCount)+String(i);
            if(cWordArray[i]==letter){
                document.getElementById(lid).style.backgroundColor = "green";
            }
            else if(!(cWordArray.includes(letter))){
                document.getElementById(lid).style.backgroundColor = "gray";
            }
            }

            for(let i = 0; i <5; i++){//check yellow | for every letter
                let letter  = line[i];
                let lid =  "l"+String(wordCount)+String(i);
                let color = getComputedStyle(document.getElementById(lid)).backgroundColor; //getComputedStyle(document.getElementById(lid)).
                //console.log("i is:" + line [i]);
                if(color === 'rgb(255, 255, 255)'){// has yet to be marked
                    let x = 0; //# of this letter
                    let y = 0; //how many accounted for
                    //console.log(getComputedStyle(document.getElementById("l01")).backgroundColor);
                    for(let j = 0; j <5; j++){
                        //console.log("j is:" + j + cWordArray[j]);
                        let lid2 = "l"+String(wordCount)+String(j);
                        if (letter == cWordArray[j]){
                            x++;
                            //console.log("xup");
                        }
                        if(letter == line[j] &&(getComputedStyle(document.getElementById(lid2)).backgroundColor === 'rgb(0, 128, 0)' || getComputedStyle(document.getElementById(lid2)).backgroundColor === 'rgb(255, 255, 0)')){
                            y++;
                            //console.log("yup");
                        }
                    }
                    if(x > y){//decides yellow
                        document.getElementById(lid).style.backgroundColor = "yellow";
                        //console.log("making yellow " + letter);
                    }
                    else{//decides gray
                        document.getElementById(lid).style.backgroundColor="gray";
                        //console.log("making gray " + letter);
                    }
                }
        }
        ...
        letterCount = 0;
        //keyboard changing:
        for(let i  = 0; i < 5; i++){//mark green and gray
            let letter  = line[i];
            let lid = determineID(line[i]);
            if(cWordArray[i]==letter){
                //console.log(lid);
                dict[lid] = "green";
                //console.log(letter + " green");
            }
            else if(!(cWordArray.includes(line[i]))){
                dict[lid]  = "gray";
                //console.log(letter + " gray");
            }
        }
        restoreLetterButtonColor();
        for(let i  = 0; i < 5; i++){
            let letter  = line[i];
            let lid = determineID(line[i]);
            if(getComputedStyle(document.getElementById(lid)).backgroundColor==='rgb(255, 255, 255)'){
                dict[lid]="yellow";
            }
        } 
        restoreLetterButtonColor();
        ...
        if(inputWord==cWord){
            document.getElementById("infoText").innerHTML = "Jūs uzvarējāt!";
        ...
        document.getElementById("infoText").innerHTML = "Vārds nav pieņemts";
    }
}
function deleteLetter(){
    letterCount  = Math.max(letterCount - 1, 0);
    //console.log(letterCount);
    line[letterCount] = '-';
    let lid =  "l"+String(wordCount)+String(letterCount);
...
function logHoveredButton() {
            if (currentButton && currentButton.style.backgroundColor == "red") {
                if(mistakes > Math.min(4, (8-diff))){
                    mistakes = 0;
                    blankWord();
                }
                else{
...
setInterval(logHoveredButton, 125);
...
function blankWord(){
    if(canMistake){
        for (let i  = 0; i < 5; i++){
            let lid = "l"+String(wordCount)+String(i);
            document.getElementById(lid).style.backgroundColor = "black";
        }
        WCIncrease();
        letterCount = 0;
        mistakes = 0;
        canMistake = 0;
        let mi = setInterval(function(){canMistake = 1; clearInterval(mi);}, 1500);//grace period
    }
}
...
function WCIncrease(){
        ...
        if(wordCount  < 6){
            clearInterval(pattern);
            ph = 0;
            let patId = "pat" + String(Math.floor(Math.random()*6)+1)+"()";//change later once more pats
            console.log(patId);
            pattern = setInterval(patId, it/diff);
        }
        else{
            //begin loss sequence
            document.getElementById("infoText").innerHTML = "Jūs zaudējāt!";
            restoreLetterButtonColor();
            clearInterval(pattern);
            gameLost = 1;
        }
...
function blinkButton(id) {
    ...
    setTimeout(restoreLetterButtonColor, 150/diff);
    setTimeout(function() {button.style.backgroundColor = "red";}, 300/diff);
}
function pat1() { // horizontal lines alternating
    // console.log(ph);
    switch(ph) {
        case 0:
            for(let i = 0; i < 2; i++) {
                for(let j = 0; j < 7; j++) {
                    let id = String(i * 2) + String(j);
                    blinkButton(id);
                }
            }
            break;
...
function pat2(){//center squeeze, it isnt efficiently written but i dont care
    switch(ph){
        case 0:
            for(let i  = 0; i < 4; i++){
                let id = String(i)+String(0);
                blinkButton(id);
                id = String(i)+String(6);
                blinkButton(id);
            }
            break;
        case 1:
            for(let i  = 0; i < 4; i++){
                ...
                id = String(i)+String(6);
                blinkButton(id);
                id = String(i)+String(1);
                blinkButton(id);
                id = String(i)+String(5);
                blinkButton(id);
            }
            break;
        case 2:
            for(let i  = 0; i < 4; i++){
                ...
                id = String(i)+String(5);
                blinkButton(id);
                id = String(i)+String(2);
                blinkButton(id);
                id = String(i)+String(4);
                blinkButton(id);
            }
            break;
        case 3:
...
function pat4(){//two lines going together
    restoreLetterButtonColor();
    switch(ph){
        case 0 :
            for(let i = 0; i < 3; i++){
                for(let j = 0 ; j < 4; j++){
                    let id = String(j)+String(i*3);
                    blinkButton(id);
                }
            }
            break;
...
function pat5(){//up and down platforms
    restoreLetterButtonColor();
    function t1(){
        blinkButton(String(ph)+String(0));
        blinkButton(String(ph)+String(1));
        blinkButton(String(ph)+String(5));
        blinkButton(String(ph)+String(6));

        blinkButton(String(3-ph)+String(2));
        blinkButton(String(3-ph)+String(3));
        blinkButton(String(3-ph)+String(4));
    }
    function t2(){
        blinkButton(String(6-ph)+String(0));
        blinkButton(String(6-ph)+String(1));
        blinkButton(String(6-ph)+String(5));
        blinkButton(String(6-ph)+String(6));

        blinkButton(String(ph-3)+String(2));
        blinkButton(String(ph-3)+String(3));
        blinkButton(String(ph-3)+String(4));
    }
    switch(ph){
        case 0:
            t1();
...
function pat6(){
    ...
    if(ph == 3){
        for(let i  = 0; i < 4; i++){
            for(let j = 0; j < 7; j++){
                let id = String(i)+String(j);
                if(!(getComputedStyle(document.getElementById(id)).backgroundColor === 'rgb(238, 130, 238)')){
                    blinkButton(id);
                }
            }
        }
...
//let pattern = setInterval(patId, it/difficulty);

let pattern;
document.getElementById("container1").addEventListener('mouseenter', () => {
    if(!gameWon && !gameLost){
        //console.log(patId + it + diff);
        let patId = "pat" + String(Math.floor(Math.random()*6)+1)+"()";//change later once more pats
        console.log(patId);
        pattern = setInterval(patId, it/diff);
...
document.getElementById("container1").addEventListener('mouseleave', () => {
    if(!gameWon && !gameLost){
        document.getElementById("infoText").innerHTML= "Novietojiet kursoru uz tastatūras!"
        blankWord();
        clearInterval(pattern);
        ph = 0;
    }
